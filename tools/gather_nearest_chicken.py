"""Pick one worker and gather a chicken (OpenEnv proxy).

This is a convenience wrapper around the "gather" command using snapshot-based
target selection.
...
Usage:
  API_BASE=http://127.0.0.1:8001 python tools/gather_nearest_chicken.py --player-id 1
"""
...
def main() -> None:
    ...
    if best is None:
        raise SystemExit("No chicken found")

    print(f"worker={worker_id} chicken={best} d2={best_d2}")
