"""Pick one worker and gather nearest stone mine (OpenEnv proxy).
...
Usage:
  API_BASE=http://127.0.0.1:8001 python tools/gather_nearest_stone.py --player-id 1
"""
...
def main() -> None:
    ...
    for sid, ent in entities.items():
        ...
        if "gaia" not in t:
            continue
        if "stone" not in t and "rock" not in t:
            continue
        tid = int(sid)
        tpos = _pos(snap, tid)
        if not tpos:
    ...
    if best is None:
        raise SystemExit("No stone mine found")

    print(f"worker={worker_id} stone={best} d2={best_d2}")
